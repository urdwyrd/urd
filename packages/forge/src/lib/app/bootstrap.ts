...
export async function bootstrap(): Promise<() => void> {
  ...
  const unsubProjectOpen = bus.subscribe('project.opened', async (payload) => {
    ...
    try {
      ...
      await scanDir(path);
      // Load per-project settings overrides from .forge/project.json
      try {
        const projectJsonPath = `${path}/.forge/project.json`;
        const projectJsonExists = await fileSystem.exists(projectJsonPath);
        if (projectJsonExists) {
          const raw = await fileSystem.readFile(projectJsonPath);
          const parsed = JSON.parse(raw);
          if (parsed.settings && typeof parsed.settings === 'object') {
            appSettings.setProjectOverrides(parsed.settings);
          }
        }
      } catch (err) {
  ...
  // 8d. Debug commands — registered after compiler service is available
  commandRegistry.register({
    id: 'forge.debug.loadStressFixture',
    title: 'Load Stress Fixture (50k entities)',
    category: 'Debug',
    execute: async () => {
      if (compilerService instanceof MockCompilerService) {
        const output = await compilerService.loadFixture('stress-50k-entities');
        const resolved = compilerCache.resolve(output);
        const hashes: Record<string, string> = {};
        for (const chunk of output.chunks) hashes[chunk.name] = chunk.contentHash;
        projectionRegistry.updateSource(resolved, hashes);
      }
      return null;
    },
  });
  ...
  commandRegistry.register({
    id: 'forge.debug.loadStressDiagnostics',
    title: 'Load Stress Fixture (huge diagnostics)',
    category: 'Debug',
    execute: async () => {
      if (compilerService instanceof MockCompilerService) {
        const output = await compilerService.loadFixture('stress-huge-diagnostics');
        const resolved = compilerCache.resolve(output);
        const hashes: Record<string, string> = {};
        for (const chunk of output.chunks) hashes[chunk.name] = chunk.contentHash;
        projectionRegistry.updateSource(resolved, hashes);

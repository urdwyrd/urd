<script lang="ts">
  /**
   * WriteSpreadsheet — table view of all property writes in the world.
   */
  ...
  import type { WriteRow } from '$lib/app/projections/write-table';
  ...
  const columns: ColumnDefinition<WriteRow>[] = [
    ...
    { key: 'valueExpr', label: 'Expression', width: '120px' },
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<WriteRow[]>('urd.projection.writeTable') ?? [];
  }
  ...
  function handleRowClick(row: WriteRow, index: number): void {
    selectedIndex = index;
  ...
  function handleRowDoubleClick(row: WriteRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter writes\u2026" />
  <div class="forge-spreadsheet-zone__table">
    <VirtualTable
      ...
      emptyMessage="No property writes"
      rowKey={(row) => row.id}

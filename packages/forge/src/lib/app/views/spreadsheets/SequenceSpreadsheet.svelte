<script lang="ts">
  /**
   * SequenceSpreadsheet — table view of section sequences in the world.
   */
  ...
  import type { SequenceRow } from '$lib/app/projections/sequence-table';
  ...
  let rawRows: SequenceRow[] = $state([]);
  let sort: SortState | null = $state(untrack(() => zoneState?.sort ?? null));
  let filterText: string = $state(untrack(() => zoneState?.filterText ?? ''));
  let selectedIndex: number = $state(-1);

  const columns: ColumnDefinition<SequenceRow>[] = [
    { key: 'sectionName', label: 'Section', width: '200px' },
    { key: 'jumpCount', label: 'Jumps', width: '80px', align: 'right' },
    { key: 'choiceCount', label: 'Choices', width: '80px', align: 'right' },
    { key: 'isTerminal', label: 'Terminal', width: '80px' },
  ...
  let filteredRows = $derived.by(() => {
    ...
    if (filterText) {
      ...
      rows = rows.filter((r) => r.sectionName.toLowerCase().includes(lower));
    }
    if (sort) {
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<SequenceRow[]>('urd.projection.sequenceTable') ?? [];
  }
  ...
  function handleRowClick(row: SequenceRow, index: number): void {
    selectedIndex = index;
    selectionContext.select([{ kind: 'section', id: row.sectionId, label: row.sectionName, data: { file: row.file, line: row.line } }], zoneId);
  }

  function handleRowDoubleClick(row: SequenceRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter sequences\u2026" />

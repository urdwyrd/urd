<script lang="ts">
  /**
   * LocationSpreadsheet — table view of all locations in the world.
   */
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<LocationRow[]>('urd.projection.locationTable') ?? [];
  }
  ...
  function handleRowClick(row: LocationRow, index: number): void {
    selectedIndex = index;
    selectionContext.select([{ kind: 'location', id: row.id, label: row.name, data: { file: row.file, line: row.line } }], zoneId);
  }

  function handleRowDoubleClick(row: LocationRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter locations\u2026" />

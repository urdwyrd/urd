<script lang="ts">
  /**
   * TypeSpreadsheet — table view of all types in the world.
   */
  ...
  import type { TypeRow } from '$lib/app/projections/type-table';
  ...
  let rawRows: TypeRow[] = $state([]);
  let sort: SortState | null = $state(untrack(() => zoneState?.sort ?? null));
  let filterText: string = $state(untrack(() => zoneState?.filterText ?? ''));
  let selectedIndex: number = $state(-1);

  const columns: ColumnDefinition<TypeRow>[] = [
    { key: 'name', label: 'Name', width: '180px' },
    { key: 'entityCount', label: 'Entities', width: '100px', align: 'right' },
    { key: 'propertyCount', label: 'Properties', width: '100px', align: 'right' },
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<TypeRow[]>('urd.projection.typeTable') ?? [];
  }
  ...
  function handleRowClick(row: TypeRow, index: number): void {
    selectedIndex = index;
    selectionContext.select([{ kind: 'type', id: row.id, label: row.name, data: { file: row.file, line: row.line } }], zoneId);
  }

  function handleRowDoubleClick(row: TypeRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter types\u2026" />

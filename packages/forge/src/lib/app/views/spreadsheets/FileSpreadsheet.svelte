<script lang="ts">
  /**
   * FileSpreadsheet — table view of all source files with symbol and diagnostic counts.
   */
  ...
  import type { FileRow } from '$lib/app/projections/file-table';
  ...
  let rawRows: FileRow[] = $state([]);
  let sort: SortState | null = $state(untrack(() => zoneState?.sort ?? null));
  let filterText: string = $state(untrack(() => zoneState?.filterText ?? ''));
  let selectedIndex: number = $state(-1);

  const columns: ColumnDefinition<FileRow>[] = [
    { key: 'filePath', label: 'File', width: '280px' },
    { key: 'symbolCount', label: 'Symbols', width: '80px', align: 'right' },
    { key: 'errorCount', label: 'Errors', width: '70px', align: 'right' },
    { key: 'warningCount', label: 'Warnings', width: '80px', align: 'right' },
  ];
  ...
  let filteredRows = $derived.by(() => {
    ...
    if (filterText) {
      ...
      rows = rows.filter((r) => r.filePath.toLowerCase().includes(lower));
    }
    if (sort) {
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<FileRow[]>('urd.projection.fileTable') ?? [];
  }
  ...
  function handleRowClick(_row: FileRow, index: number): void {
    selectedIndex = index;
  }

  function handleRowDoubleClick(row: FileRow): void {
    if (row.filePath) {
      navigationBroker.navigate({
        targetViewId: 'urd.codeEditor',
        params: { path: row.filePath, line: 1 },
      });
    }
  }
</script>
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter files\u2026" />

<script lang="ts">
  /**
   * EntitySpreadsheet — table view of all entities in the world.
   */
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<EntityRow[]>('urd.projection.entityTable') ?? [];
  }
  ...
  function handleRowClick(row: EntityRow, index: number): void {
    selectedIndex = index;
    selectionContext.select([{ kind: 'entity', id: row.id, label: row.name, data: { file: row.file, line: row.line } }], zoneId);
  }

  function handleRowDoubleClick(row: EntityRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter entities\u2026" />

<script lang="ts">
  /**
   * RuleSpreadsheet — table view of all rules in the world.
   */
  ...
  import type { RuleRow } from '$lib/app/projections/rule-table';
  ...
  let rawRows: RuleRow[] = $state([]);
  let sort: SortState | null = $state(untrack(() => zoneState?.sort ?? null));
  let filterText: string = $state(untrack(() => zoneState?.filterText ?? ''));
  let selectedIndex: number = $state(-1);

  const columns: ColumnDefinition<RuleRow>[] = [
    { key: 'ruleId', label: 'Rule ID', width: '200px' },
    { key: 'conditionCount', label: 'Conditions', width: '100px', align: 'right' },
    { key: 'effectCount', label: 'Effects', width: '90px', align: 'right' },
  ...
  let filteredRows = $derived.by(() => {
    ...
    if (filterText) {
      ...
      rows = rows.filter((r) => r.ruleId.toLowerCase().includes(lower));
    }
    if (sort) {
  ...
  function refreshData(): void {
    rawRows = projectionRegistry.get<RuleRow[]>('urd.projection.ruleTable') ?? [];
  }
  ...
  function handleRowClick(row: RuleRow, index: number): void {
    selectedIndex = index;
    selectionContext.select([{ kind: 'rule', id: row.id, label: row.ruleId, data: { file: row.file, line: row.line } }], zoneId);
  }

  function handleRowDoubleClick(row: RuleRow): void {
...
<div class="forge-spreadsheet-zone">
  <FilterBar {filterText} onFilterChange={handleFilterChange} placeholder="Filter rules\u2026" />

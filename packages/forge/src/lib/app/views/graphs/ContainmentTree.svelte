<script lang="ts">
  /**
   * Containment Tree — world → locations → entities hierarchy.
   * Consumes the containmentTree projection via GraphCanvas.
   */
  ...
  function refreshData(): void {
    graphData = projectionRegistry.get<ForgeGraphData>('urd.projection.containmentTree');
  }
...
{#if graphData && graphData.nodes.length > 0}
  <GraphCanvas
    ...
    emptyMessage="No world content to display"
  />
{:else}
  <div class="forge-graph-view__empty">
    <p>No containment hierarchy to display</p>
    <p class="forge-graph-view__hint">Locations and entities appear after successful compilation</p>
  </div>
{/if}

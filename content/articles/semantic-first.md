...
MCP, the emerging standard for giving AI models structured access to external tools, is the natural delivery mechanism. An Urd world exposed through MCP would provide the obvious runtime operations (`get_state`, `get_available_actions`, `perform_action`) alongside FactSet query tools (`get_exit_graph`, `get_property_dependencies`, `get_choice_conditions`, `get_reachable_locations`). The FactSet is already serialised to JSON for the playground's analysis panel. Wrapping it in MCP tools is mechanical. The hard part, extracting, normalising, and cross-referencing the semantic relationships, is already done at compile time.
